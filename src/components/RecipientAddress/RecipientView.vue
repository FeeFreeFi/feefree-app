  ...
  <ZModalView title="Recipient Address" :on-close="onClose">
    <div class="flex flex-col px-4 pb-4">
      <div class="flex overflow-hidden">
        ...
        <div>
          <ZButton v-if="!input || focus" class="px-0 !rounded-l-none w-12 sm:w-[68px] h-full text-xs sm:text-sm" @click="onPaste">
            Paste
          </ZButton>
          <ZButton v-else class="px-0 !rounded-l-none w-12 sm:w-[68px] h-full text-xs sm:text-sm" @click="onClear">
            Clear
          </ZButton>
        </div>
      </div>
      ...
      <div class="mt-6">
        <ZButton v-if="recipient" class="w-full h-10 sm:h-12" aria-label="Remove Recipient Addresss" @click="onClear">
          Remove Recipient Addresss
        </ZButton>
        <ZButton v-else class="w-full h-10 sm:h-12" :disabled="!input || isSame || !checked || !isValid" aria-label="Confirm Recipient Addresss" @click="onOk">
