...
<template>
  <router-link class="sm:flex-initial flex-1 flex-center no-underline" :to="to" #="{ href }">
    <n-button v-if="screen.sm" class="px-4 h-9" text :aria-label="label">
      <n-text class="font-medium hover:text-font text-base" :class="[isActive(href) ? 'text-font' : ' text-font-70']">
        {{ label }}
      </n-text>
    </n-button>
    <div v-else class="flex-col flex-center gap-1 h-12">
      ...
      <n-text class="text-xs transition-all" :class="[isActive(href) ? 'text-primary' : 'text-font-70']">
        {{ label }}
      </n-text>
    </div>
  </router-link>
