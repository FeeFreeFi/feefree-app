<template>
  <ZModalView class="h-[480px]" title="Connect Wallet" :on-close="onClose">
    <div class="h-full relative">
      <div v-if="providers.length > 0" class="size-full absolute top-0 left-0">
        <n-scrollbar class="p-4">
          <div class="flex flex-col gap-2">
            <WalletItem v-for="item, index in providers" :key="index" :name="item.info.name" :icon="item.info.icon" :active="isActive(item)" :recent="isRecent(item)" @click="() => onConnect(item)" />
          </div>
        </n-scrollbar>
      </div>
      <NoWallet v-else />
...
const onConnect = async wallet => {
  try {
    connecting(wallet)
    await wait(300)
    const success = await connect(wallet.provider, wallet.info, appChainId.value)
    if (success) {
