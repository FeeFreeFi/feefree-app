...
/** @type {{[chainId:number]: {chainId:number, address:string, poolOld?:import('@/types').PoolMeta & {key:import('@/types').PoolKey}, poolLegacy: import('@/types').PoolMeta & {key:import('@/types').PoolKey, liquidity:import('@/types').Token}, tokens:(import('@/types').Token & {origin:string})[], fee:bigint}}}*/
const CONFIGS = {
  [CHAIN_ID_ZORA]: {
 ...
 */
export const getMigration = chainId => CONFIGS[chainId]

/**
 * @param {import('@/types').Token} token
 */
export const unexchange = async token => {
  const { chainId } = token
 ...
 */
export const removeLiquidity = async (chainId, key) => {
  const publicClient = getPublicClient(chainId)
  const walletClient = getWalletClient()
 ...
 * @param {{currency0:string, currency1:string, fee:number, tickSpacing:number, hooks:string}} key
 */
export const migrateLiquidity = async (chainId, key) => {
  const publicClient = getPublicClient(chainId)
  const walletClient = getWalletClient()
  const { address: migration } = getMigration(chainId)
