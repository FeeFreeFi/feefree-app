  ...
  <div class="flex flex-col flex-1">
    ...
    <div class="flex flex-col flex-1 gap-2 bg-card/40 mt-4 rounded">
      <div class="flex items-center gap-1 bg-card px-3 rounded-t h-9 font-semibold">
        <n-text class="w-14 text-center" :depth="1">
          Points
        </n-text>
        <n-text class="w-[100px] text-center" :depth="1">
          Date
        </n-text>
        <n-text class="flex-1 sm:w-20 text-center" :depth="1">
          Reason
        </n-text>
        <n-text v-if="screen.sm" class="flex-1 text-center" :depth="1">
          Meta
        </n-text>
      </div>
      <div v-if="list.length > 0">
        <div v-for="item, index in list" :key="index" class="flex items-center gap-1 hover:bg-card/60 px-3 h-9 transition-colors">
          <n-text class="w-14 font-medium text-sm text-center">
            {{ item.value }}
          </n-text>
          <n-text class="w-[100px] text-xs text-center" :depth="1">
            {{ dayjs(item.date).fromNow() }}
          </n-text>
          <div class="flex-1 sm:w-20 text-xs text-center">
            <n-text v-if="screen.sm" :depth="1">
              {{ NAMES[item.reason] }}
            </n-text>
            <div v-else>
              ...
              <div v-else>
                <n-text class="flex-1 text-xs text-center" :depth="1">
                  {{ item.meta.remark }}
                </n-text>
              </div>
            </div>
