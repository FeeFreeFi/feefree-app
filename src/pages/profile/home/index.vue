    ...
    <div v-if="account" class="flex flex-col flex-1 gap-8">
      <div class="flex flex-col gap-4">
        <div class="flex-y-center gap-2">
          <n-text class="w-14" :depth="1">
            Account
          </n-text>
          <div class="flex-y-center gap-2">
            <n-text class="font-medium text-base">
              {{ shortString(account, 10, -8) }}
            </n-text>
        ...
        <div class="flex-y-center gap-2">
          ...
          <div class="flex-y-center gap-2">
            <n-progress class="!w-[200px] shrink-0" type="line" :percentage="percentage">
              <n-text class="text-sm" depth="1">
                {{ profile?.exp || 0 }}/{{ profile?.nextExp || 0 }}
              </n-text>
            </n-progress>
          </div>
        </div>
      ...
      <div class="flex-1" />
      <ZButton class="w-full h-10 sm:h-12" aria-label="Logout" @click="onLogout">
        Logout
      </ZButton>
    </div>
    <div v-else class="flex-1 flex-center">
      <ZButton class="w-60 h-10 sm:h-12" aria-label="Connect wallet" @click="openWalletConnector">
        Connect Wallet
      </ZButton>
    </div>
  </div>
