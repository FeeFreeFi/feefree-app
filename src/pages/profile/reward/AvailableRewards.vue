  ...
  <div>
    ...
    <div class="flex flex-col gap-2 bg-card/40 mt-4 rounded">
      <div class="flex items-center gap-1 bg-card px-3 rounded-t h-9 font-semibold">
        ...
        <n-text v-if="screen.sm" class="w-24 text-center" :depth="1">
          Update at
        </n-text>
        <n-text class="flex-1 pr-[10px] text-right" :depth="1">
          Action
        </n-text>
      </div>
      <div v-for="item, index in list" :key="index" class="flex items-center gap-1 hover:bg-card/60 px-3 h-10 transition-colors">
        ...
        <div class="flex flex-1 justify-end">
          <ZButton class="w-16 !font-normal text-xs" size="small" :disabled="claiming || !item.valid" @click="() => onClaim(item)">
            Claim
          </ZButton>
        </div>
      </div>
