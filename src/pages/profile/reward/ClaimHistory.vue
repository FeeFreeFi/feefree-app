  ...
  <div class="flex flex-col flex-1">
    ...
    <div class="flex flex-col flex-1 gap-2 bg-card/40 mt-4 rounded">
      <div class="flex items-center gap-1 bg-card px-3 rounded-t h-9 font-semibold">
        <n-text class="w-[72px]" :depth="1">
          Chain
        </n-text>
        <n-text class="w-28 text-center" :depth="1">
          Value
        </n-text>
        <n-text class="flex-1 sm:w-24 text-center" :depth="1">
          Timestamp
        </n-text>
        <n-text v-if="screen.sm" class="flex-1 text-right" :depth="1">
          Txn hash
        </n-text>
      </div>
      <div v-if="list.length > 0">
        <div v-for="item, index in list" :key="index" class="flex items-center gap-1 hover:bg-card/60 px-3 h-9 transition-colors">
          ...
          <div class="flex-1 sm:w-24 text-xs text-center">
            <n-text v-if="screen.sm" class="flex-1 sm:w-24 text-xs text-center" :depth="1">
              {{ item.claimedAt ? dayjs(item.claimedAt).fromNow() : "N/A" }}
            </n-text>
