<template>
  <div class="flex flex-col text-sm sm:text-base">
    <div class="mx-auto relative w-full sm:w-[520px]">
      <n-tabs class="h-full relative [&>.n-tabs-nav]:hidden" :value="tab" pane-wrapper-class="h-full" pane-class="h-full relative !p-0" animated>
        <n-tab-pane :name="TAB_ADD_LIQUIDITY">
          <!-- add liquidity -->
          <ZSectionView>
            <!-- header -->
            <div class="h-10 flex-y-center justify-between">
              <n-text class="text-xl sm:text-2xl font-medium">Add Liquidity</n-text>
            </div>
          ...
          <ZSectionView>
            ...
            <div class="my-4 flex flex-col gap-6">
              ...
              <div class="bg-block px-3 sm:px-4 flex flex-col">
                ...
                <div class="py-4 flex justify-between gap-2">
                  ...
                  <div class="flex">
                    <ZTokenBalance :token="pool.currencyLiquidity" :balance="quoteData?.liquidity || 0n" :dp="6" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div v-else-if="!approved" class="flex gap-2">
                <n-button v-if="!approved0" class="primary-btn flex-1" :disabled="approving0" :loading="approving0" type="primary" strong :aria-label="`Unlock ${currency0.symbol}`" @click="() => onApproval(true)">Unlock {{ currency0.symbol }}</n-button>
                <n-button v-if="!approved1" class="primary-btn flex-1" :disabled="approving1" :loading="approving1" type="primary" strong :aria-label="`Unlock ${currency1.symbol}`" @click="() => onApproval(false)">Unlock {{ currency1.symbol }}</n-button>
              </div>
              <n-button v-else class="primary-btn" :disabled="adding" :loading="adding" type="primary" block strong aria-label="Swap" @click="onAdd">{{ signing ? 'Waiting For Wallet Confirmation' : 'Add Liquidity' }}</n-button>
            </div>
          </ZSectionView>
        </n-tab-pane>
      </n-tabs>
...
import { allowance, approve } from "@/hooks/useCurrency"
...
import { addLiquidity, quoteAddLiquidity, getAmount0FromSqrtPrice, getAmount1FromSqrtPrice } from "@/hooks/useSwap"
...
const TAB_ADD_LIQUIDITY = "add-liquidity"
const TAB_REVIEW = "review"

const tab = ref(TAB_ADD_LIQUIDITY)
...
const currency0 = computed(() => props.pool.currency0)
const currency1 = computed(() => props.pool.currency1)

const inputAmount0 = ref("")
const inputAmount1 = ref("")
const amount0 = computed(() => parseAmount(inputAmount0.value || 0, currency0.value.decimals))
const amount1 = computed(() => parseAmount(inputAmount1.value || 0, currency1.value.decimals))
...
const requireSwitchChain = computed(() => chainId.value !== props.pool.chainId)
const canReview = computed(() => true)
...
const updateQuoteData = async () => {
  ...
  const id = currency0.value.chainId
  const publicClient = getPublicClient(id)
  const address = getRouterAddress(id)

  try {
    const result = await quoteAddLiquidity(publicClient, address, currency0.value.address, currency1.value.address, amount0.value, amount1.value)
...
const checkAllowance = async () => {
  await Promise.all([
    doCheckAllowance(true),
    doCheckAllowance(false)
  ])
}

const doCheckAllowance = async isZero => {
  const currency = isZero ? currency0.value : currency1.value
  const amount = isZero ? amount0.value : amount1.value

  const { address, chainId: id } = currency
  const publicClient = getPublicClient(id)
  const spender = getRouterAddress(id)
  const val = await allowance(publicClient, address, account.value, spender).catch(() => 0n)
  const allow = val >= amount

  if (isZero) {
    approved0.value = allow
  } else {
    approved1.value = allow
  }
}
...
const onMax0 = () => {
  inputAmount0.value = toAmount(props.balance0, currency0.value.decimals)
  onAmount0Blur()
}

const onMax1 = () => {
  inputAmount1.value = toAmount(props.balance1, currency0.value.decimals)
  onAmount1Blur()
}
...
const onAmount0Blur = () => {
  ...
  inputAmount1.value = toAmount(getAmount1FromSqrtPrice(props.poolState.sqrtPriceX96, amount0.value), currency0.value.decimals)
...
const onAmount1Blur = () => {
  ...
  inputAmount0.value = toAmount(getAmount0FromSqrtPrice(props.poolState.sqrtPriceX96, amount1.value), currency1.value.decimals)
  debounceUpdateQuoteData()
}
...
const onApproval = async isZero => {
  const currency = isZero ? currency0.value : currency1.value
  const balance = isZero ? props.balance0 : props.balance1
  const { symbol, address, chainId:id } = currency

  const spender = getRouterAddress(id)

  if (isZero) {
    approving0.value = true
  } else {
    approving1.value = true
  }
  signing.value = true
...
const onAdd = async () => {
  ...
  const _amount0 = toAmount(amount0.value, currency0.value.decimals, currency0.value.dp)
  const _amount1 = toAmount(amount1.value, currency1.value.decimals, currency1.value.dp)
  const content = `Add liquidity for ${_amount0} ${currency0.value.symbol} and ${_amount1} ${currency1.value.symbol}`
  ...
  try {
    const tx = await addLiquidity(
      { publicClient, walletClient },
      address,
      currency0.value.address,
      currency1.value.address,
      amount0.value,
      amount1.value,
      amount0Min,
      amount1Min,
