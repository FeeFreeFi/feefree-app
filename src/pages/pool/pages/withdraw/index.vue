      ...
      <div class="flex flex-col flex-1 gap-10 lg:bg-tab lg:p-6 lg:rounded">
        ...
        <div>
          <ActionButton :chain-id="pool.chainId" :chains="supportedChains">
            ...
            <ZButton v-else-if="!approved" class="w-full h-10 sm:h-12" :disabled="approving" :loading="approving" :aria-label="`Approve ${getPoolName(pool)}`" @click="onApproval">
              Approve {{ getPoolName(pool) }}
            </ZButton>
            <ZButton v-else class="w-full h-10 sm:h-12" :loading="withdrawing" aria-label="Deposit" @click="onWithdraw">
              Withdraw
            </ZButton>
          </ActionButton>
        </div>
