...
function getAccountHash(account: string) {
  return base58.encode(fromHexString(hashMessage(account.toLowerCase()).slice(2)))
}

export function getAccountId(account: string) {
  return getAccountHash(account).slice(0, 8)
}

export function getAccountReferral(account: string) {
  return getAccountHash(account).slice(-8)
}

export function isSelfAccount(account: string, id: string) {
  return getAccountId(account) === id
}

export function isSelfReferral(account: string, referral: string) {
